...
#define R 576 //SHA3_224 : 1152   SHA3_256 : 1088  SHA3_384 : 832  SHA3_512 : 576
...
public:
    PUCHAR Hash_Data;
    SHA3_512_Data(PUCHAR M, ULONG32 Byte_Len);
    ~SHA3_512_Data();
    VOID HASH();
};

SHA3_512_Data::~SHA3_512_Data() {
    free(Mc);
    free(M);
    free(Hash_Data);
}

SHA3_512_Data::SHA3_512_Data(PUCHAR M, ULONG32 Byte_Len) {
...
VOID SHA3_512_Data::theat(PUCHAR S) {
...
VOID SHA3_512_Data::rho(PUCHAR S) {
    PUCHAR Sc = (PUCHAR)malloc(B / 8);
    memset(Sc, 0, B / 8);
    //memcpy(Sc, S, B / 8);
    for (int z = 0; z < W; z++) {
...
VOID SHA3_512_Data::pi(PUCHAR S) {
    PUCHAR Sc = (PUCHAR)malloc(B / 8);
    int j = 0;
    memset(Sc, 0, B / 8);
    for (int x = 0; x < 5; x++) {
...
VOID SHA3_512_Data::chi(PUCHAR S) {
    PUCHAR Sc = (PUCHAR)malloc(B / 8);
    memset(Sc, 0, B / 8);
    for (int x = 0; x < 5; x++) {
        for (int y = 0; y < 5; y++) {
...
VOID SHA3_512_Data::iota(PUCHAR S, int Ir) {
    PUCHAR Sc = (PUCHAR)malloc(B / 8);
    memcpy(Sc, S, B / 8);
    UCHAR RC[W] = { 0 };
    for (int j = 0; j <= L; j++) {
...
VOID SHA3_512_Data::Rnd(PUCHAR S, int Ir) {
    theat(S);
    rho(S);
    pi(S);
    chi(S);
...
VOID SHA3_512_Data::KECCAK_P(PUCHAR S) {
    for (int ir = 12 + 2 * L - Nr; ir <= 12 + 2 * L - 1; ir++) {
        Rnd(S, ir);
    }
}

VOID SHA3_512_Data::HASH() {
