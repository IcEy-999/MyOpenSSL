...
//阉割版：正常SHA384 可散列2^128 Bit长的数据 ， 这个只可散列 2^64 Bit长的数据
//80个初始常量
ULONG64 K[] = {
...
typedef struct _SHA384_Data {
    ...
    PUCHAR Hash_Data;
    _SHA384_Data(PUCHAR M);
}SHA384_Data, * PSHA384_Data;


SHA384_Data::_SHA384_Data(PUCHAR M) {
    this->M_Len_Bit = MyMemlen_Bit(M);
    ULONG64 Len_bf = this->M_Len_Bit;
    ULONG64 bl = 0;
    int mod = this->M_Len_Bit % 1024;
    if (mod + 128 + 8 <= 1024 && mod > 0) {
        while ((Len_bf + 128 + 8) % 1024 != 0) {//留一个8bit and 128bit整数
            Len_bf += 8;
        }
    }
    else {
        while ((Len_bf) % 1024 != 0) {//补长度，1024
            Len_bf += 8;
        }
        Len_bf += 8;
        while ((Len_bf + 128 + 8) % 1024 != 0) {//留一个8bit and 128bit整数
    ...
    memcpy(&buffer[(Len_bf + 8 + 64) / 8], bl_fp, 8);//原先SHA384 可以散列 2^128 长Bit的数据，但是我这里阉割了，变为和SHA384一样的2^64Bit长
    this->Mc = buffer;
...
BOOLEAN Mc_To_W(PSHA384_Data Data) {
    ULONG64 B_i = Data->Mc_Len_Bit / 1024;
